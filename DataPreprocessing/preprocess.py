...
def make_hole(id, globalInfo, start, end):
    ...
    for idx in range(fix_start, end):
        ...
        if ENABLE_DEMODULATE:
            img_gt = cv.imread(os.path.join(globalInfo.mDemodulatePath, globalInfo.PrefixPreTM + idx_res + ".exr"), cv.IMREAD_UNCHANGED)[..., :3]
        else:
            img_gt = cv.imread(os.path.join(globalInfo.mPath, globalInfo.PrefixPreTM + idx_res + ".exr"), cv.IMREAD_UNCHANGED)[..., :3]
