...
class ExtraNet_noHistory(nn.Module):
    # a tinyer Unet which only has 3 downsample pass
    def __init__(self, n_channels, n_classes, bilinear=False, skip=True):
        super(ExtraNet_noHistory, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        self.skip = skip
...
class ExtraNet_demodulate_noHistory_SS_blend(nn.Module):
    # a tinyer Unet which only has 3 downsample pass
    def __init__(self, n_channels, n_classes, demodulate_type="glossy", upsample="bilinear", skip=True):
        super(ExtraNet_demodulate_noHistory_SS_blend, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.upsample = upsample
        self.skip = skip
        ...
        self.bn3 = nn.BatchNorm2d(16)
        self.relu3 = nn.ReLU(inplace=True)
        self.ss_conv2 = nn.Conv2d(16, 1, kernel_size=1)
    ...
    def forward(self, input):
        ...
        upsampled_cur = feature_block[:, :3, :, :]

        out = self.ss_conv1(torch.cat([high_input, feature_block], dim=1))
        out = self.relu3(self.bn3(out))
        out = self.ss_conv2(out)
        blend_weight = torch.sigmoid(out)

        highreso_color = upsampled_cur * (1 - blend_weight) + high_input * blend_weight
